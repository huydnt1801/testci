...
func (sq *SessionQuery) QueryDriver() *VehicleDriverQuery {
	query := (&VehicleDriverClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withDriver: sq.withDriver.Clone(),
...
func (sq *SessionQuery) WithDriver(opts ...func(*VehicleDriverQuery)) *SessionQuery {
	query := (&VehicleDriverClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withDriver; query != nil {
		if err := sq.loadDriver(ctx, query, nodes, nil,
			func(n *Session, e *VehicleDriver) { n.Edges.Driver = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadDriver(ctx context.Context, query *VehicleDriverQuery, nodes []*Session, init func(*Session), assign func(*Session, *VehicleDriver)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Session)
	for i := range nodes {
		fk := nodes[i].DriverID
...
func (sq *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withDriver != nil {
			_spec.Node.AddColumnOnce(session.FieldDriverID)
		}
	}
